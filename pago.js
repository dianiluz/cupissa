...
function inicializarWompiWidget() {
    const container = document.getElementById("paso-4");
    
    // Crear contenedor para Wompi si no existe
    ...
    const publicKey = "pub_prod_q69BzlCLtdFiZQEmbQFTMX9uXwr6E4Xg";
    
    wompiContainer.innerHTML = "";
    
    if (typeof WidgetCheckout === 'undefined') {
        mostrarNotificacion("Wompi no está disponible. Usa transferencia directa.");
        procesandoCompra = false;
        return;
    }
    ...
    const checkout = new WidgetCheckout({
        ...
        reference: "CUPISSA-" + Date.now(),
        publicKey: publicKey,
        redirectUrl: CONFIG.baseURL + "/pago/?success=true"
    });

    checkout.render(wompiContainer);

    // Ocultar botón de confirmar mientras se procesa Wompi
